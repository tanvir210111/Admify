...
export default function AgentStudents() {
  ...
  const fetchStudents = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/api/agent/students?search=${encodeURIComponent(search)}`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-white">My Assigned Students</h1>
          <p className="text-slate-400 text-xs sm:text-sm mt-0.5">
            Students allocated to you by your agency for admissions counseling, document checking, and application processing.
          </p>
        </div>
      ...
      >
        <div className="relative w-full max-w-md">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 rounded-xl bg-white/5 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500 transition-colors"
          />
        </div>
      ...
      >
        {loading ? (
          <div className="p-12 text-center text-slate-400 text-xs">Loading assigned caseload...</div>
        ) : students.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-slate-500 text-xs mt-1 max-w-sm mx-auto">
              Students assigned to you by your agency for admissions guidance or service orders will appear here.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/5 text-[11px] font-bold uppercase tracking-wider text-slate-400 bg-white/[0.02]">
                  ...
                  <th className="py-3.5 px-4">Destination & Course</th>
                    ...
                    <td className="py-3.5 px-4">
                      {st.activeApplication ? (
                        <span className="inline-block px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
                          {st.activeApplication.stage || "Submitted"}
                        </span>
                      ) : (
                        <span className="text-slate-500 text-[11px]">In Counseling</span>
                      )}
                    </td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
            ...
            >
              ...
              <div className="py-4 space-y-3 text-xs">
                ...
                <div className="flex justify-between py-1.5 border-b border-white/5">
                  <span className="text-slate-400">Academic Score (GPA)</span>
                  <span className="text-white font-medium">{selectedStudent.gpa || "N/A"}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-white/5">
                  <span className="text-slate-400">English Proficiency (IELTS)</span>
              ...
              <div className="pt-4 border-t border-white/10 flex justify-between items-center">
                <button
                  onClick={() => {
                    ...
                    navigate(`/agent/messages?recipient=${sId}`);
                  }}
