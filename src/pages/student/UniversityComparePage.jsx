    ...
    .map((slug) => {
      ...
      if (data) {
        return {
          ...
          livingCost: convertTextToDual(data.costs?.housingAndFood || "$15,000 / year"),
          totalEstimated: convertTextToDual(data.costs?.totalEstimated || "$50,000 / year"),
          gpaReq: data.admissionReqs?.gpa || "3.6+ Cumulative GPA",
          englishReq: data.admissionReqs?.englishProficiency || "IELTS 7.0+ / TOEFL 100+",
          deadlines: data.applicationDeadline || "Fall: Jan 5 / Nov 1",
          scholarships: data.scholarshipsList?.map((s) => s.name).join(", ") || "Institutional Aid Available",
          popularPrograms: data.programs?.map((p) => p.name).slice(0, 3).join(", ") || "Computer Science",
          logo: data.logo,
        };
      }
      return null;
