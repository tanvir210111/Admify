...
function DirectApplicationsPage() {
  ...
  const handleConfirmDeposit = (e) => {
    ...
    setTimeout(() => {
      const verified = studentService.confirmFeeDeposit(university, {
        trxId: trxInput,
        paymentMethod,
      });
      setFeeClearance(verified);
      setConfirmingDeposit(false);
      toast.success(`Fee Verified! Receipt #${verified.receiptNumber} generated by Agent.`);
    }, 500);
  };
  ...
  const handleSubmitApplication = async (e) => {
    ...
    try {
      ...
      toast.success(`Application submitted to ${university}! You can track its progress below.`);
    } catch (err) {
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-8 pb-16">
      ...
      {/* ── Business Rule Status Card: 1 Free Application ────────── */}
      <div className="p-6 sm:p-7 rounded-3xl bg-gradient-to-r from-[#07142D] via-[#0B1228] to-[#07142D] border border-cyan-500/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-xl">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-emerald-400 uppercase tracking-wider">
              {freeAppStatus.freeAvailable ? "Free Tier Benefit Active" : "Free Direct Application Consumed"}
            </span>
          </div>
          <h3 className="text-base sm:text-lg font-extrabold text-white">
            {freeAppStatus.freeAvailable
              ? "Every student receives 1 FREE application for universities with $0 Application Fee."
              : "You have used your 1 free direct application."}
          </h3>
          <p className="text-xs text-slate-300 max-w-3xl leading-relaxed">
            {freeAppStatus.freeAvailable ? (
              <>
                <strong className="text-emerald-300">Zero-fee Universities:</strong> Submitting to institutions with $0 / waived application fees is 100% free with immediate dispatch.
                <br />
                <strong className="text-cyan-300">Paid-fee Universities:</strong> If you choose a university with an institutional application fee, Admify platform fee remains free, but the university fee must be deposited through our <strong className="text-white">Admin Panel assigned Fee Agent</strong> before submission.
              </>
            ) : (
              "To submit additional direct applications across our 450+ partner universities, please purchase application credits or access full packages."
            )}
          </p>
        </div>
        ...
        <div className="shrink-0 flex items-center gap-3">
          {freeAppStatus.freeAvailable ? (
            <div className="px-4 py-2.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-center">
              <span className="text-[10px] uppercase font-bold text-emerald-300 block">Status</span>
              <span className="text-sm font-black text-emerald-400">1 Free Available</span>
            </div>
          ) : (
            <Link
              to="/student/wallet"
            ...
            >
              ...
              <span>Purchase Application Credit</span>
            </Link>
          )}
        </div>
      </div>
